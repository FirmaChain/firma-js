...
import { EncodeObject, encodePubkey, makeSignDoc, OfflineDirectSigner, Registry, TxBodyEncodeObject } from "@cosmjs/proto-signing";
...
export class SigningStargateClient extends StargateClient {
  ...
  public static async connectWithSigner(endpoint: string, signer: OfflineDirectSigner, registry: Registry): Promise<SigningStargateClient> {
 ...
 */
  ...
  public async getAccount(address: string): Promise<Account | undefined> {
    ...
    try {
      ...
      let accAddress = this.toAccAddress(address);
      var hexAccAddress = "0x01" + Buffer.from(accAddress).toString('hex');
      ...
      let path = "/abci_query?path=\"/store/acc/key\"";
      var result = await axios.get(path, { params: { data: hexAccAddress } });

      let finalData = result.data.result.response.value;
      var account = Any.decode(Buffer.from(finalData, "base64"));

      let finalAccount = accountFromAny(account);
  ...
  private makeSignerInfos(signers: ReadonlyArray<{ readonly pubkey: Any; readonly sequence: number }>, signMode: SignMode): SignerInfo[] {
    return signers.map(
  ...
  private makeAuthInfoBytes(signers: ReadonlyArray<{ pubkey: Any; sequence: number }>, feeAmount: Coin[], gasLimit: number, granter: string, signMode = SignMode.SIGN_MODE_DIRECT): Uint8Array {
    const authInfo = {
